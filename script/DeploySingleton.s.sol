...
contract DeploySingleton is Script {
    ...
    function deploySingletonForTest()
        public
        broadcast
        returns (Singleton, MultiSig, BaseRegistry, address, uint256)
    {
        //============ MULTISIG============
        ...
        RegistryFactory factory = new RegistryFactory(
            address(new BaseRegistry()), address(wrappedMultiSig), deployer, address(0x123)
        );
        ...
        returns (Singleton, MultiSig, BaseRegistry, address, uint256)
    {
        ...
        //============REGISTRY============
        RegistryFactory factory = new RegistryFactory(
            address(new BaseRegistry()), address(wrappedMultiSig), backend, ngns
        );
        wrappedMultiSig.setSingletonAndFactory(address(wrappedSingleton), address(factory));
