...
contract DeployImpl is Script {
    ...
    function upgrade() public {
        ...
        address[1] memory proxies = [address(singProxyTestnet)];
        address[1] memory newImpls = [address(newSingleton)];

        for (uint256 i = 0; i < proxies.length;) {
            address proxy = proxies[i];
            address newImpl = newImpls[i];
            if (proxy == address(multsigProxyTestnet)) {
                multsigProxyTestnet.proposeUpgrade(proxy, newImpl, true);
            } else {
                multsigProxyTestnet.proposeUpgrade(proxy, newImpl, false);
            }
            multsigProxyTestnet.validateUpgrade(newImpl);
            multsigProxyTestnet.executeUpgrade(newImpl);
