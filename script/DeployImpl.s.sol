...
contract DeployImpl is Script {
    ...
    function run() external broadcastLive {
        ...
        address[3] memory proxies =
            [address(singProxyMainnet), address(multsigproxyMainnet), address(factoryProxyMainnet)];
        address[3] memory newImpls =
            [address(newSingleton), address(newMultiSig), address(newFactory)];
